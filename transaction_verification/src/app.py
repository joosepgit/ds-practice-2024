...
class TransactionVerificationService(
    ...
    def Initialize(
        self, request: transaction_verification.InitializationRequest, context
    ):
        ...
        logging.info(
            f"Successfully initialized transaction verification with vector clock: {vector_clock} \
                      and data: {request}"
        )
        return response

    def VerifyTransaction(
        self, request: transaction_verification.TransactionVerificationRequest, context
    ):
        ...
        cached_data: tuple[dict, transaction_verification.InitializationRequest] = (
        ...
        billing_address_success, billing_address_msg = self.validate_billing_address(
        ...
        credit_card_success, credit_card_msg = self.validate_credit_card(
        ...
        fraud_detection_response: fraud_detection.FraudDetectionResponse = (
            self.detect_fraud(request.order_id.value, vector_clock)
        )
        ...
        vector_clock = self.update_vector_clock(
            request.order_id.value, dict(fraud_detection_response.vector_clock.clocks)
        )
    ...
    def validate_billing_address(
        self, billing_address: transaction_verification.BillingAddress
    ):
        # Address information
    ...
    def validate_credit_card(self, credit_card: transaction_verification.CreditCard):
        ...
        if len(credit_card.card_number) < 8 or len(credit_card.card_number) > 19:
            return (
                False,
                "Card number should contain at least 8 and at most 19 numbers.",
            )
    ...
    def detect_fraud(self, order_id: str, vector_clock: dict):
        ...
        with grpc.insecure_channel("fraud_detection:50051") as channel:
            ...
            response: fraud_detection.FraudDetectionResponse = stub.DetectFraud(
