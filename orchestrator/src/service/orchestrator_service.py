...
async def init_fraud_detection(order_id, credit_card, billing_address):
    ...
    with grpc.insecure_channel("fraud_detection:50051") as channel:
        ...
        response: fraud_detection.InitializationResponse = stub.Initialize(
            fraud_detection.InitializationRequest(
...
async def init_transaction_verification(order_id, credit_card, billing_address, items):
    ...
    with grpc.insecure_channel("transaction_verification:50052") as channel:
        ...
        response: transaction_verification.InitializationResponse = stub.Initialize(
            transaction_verification.InitializationRequest(
                order_id=order_id_proto,
                card_information=credit_card_grpc_proto,
                billing_address=billing_address_grpc_proto,
...
async def init_suggestions(order_id, books):
    ...
    with grpc.insecure_channel("suggestions:50053") as channel:
        ...
        response: suggestions.InitializationResponse = stub.Initialize(
            suggestions.InitializationRequest(
                order_id=order_id_proto, title=books[0]["name"]
            )
        )
...
async def get_suggestions(order_id: str, post_success_vector_clock: dict):
    ...
    with grpc.insecure_channel("suggestions:50053") as channel:
        stub = suggestions_grpc.SuggestionServiceStub(channel)
        response: suggestions.GetSuggestionsResponse = stub.GetSuggestions(
            suggestions.GetSuggestionsRequest(
                order_id=order_id_proto, vector_clock=post_success_vector_clock_proto
            )
        )
...
async def clear_data(order_id: str, final_vector_clock: dict):
    ...
    await asyncio.gather(
        clear_transaction_verification_data(order_id, final_vector_clock),
...
async def clear_transaction_verification_data(order_id: str, final_vector_clock: dict):
    ...
    transaction_verification_final_vector_clock_proto = (
        transaction_verification.VectorClock()
    )
    if final_vector_clock:
        transaction_verification_final_vector_clock_proto.clocks.update(
            final_vector_clock
        )
    ...
    with grpc.insecure_channel("transaction_verification:50052") as channel:
        stub = transaction_verification_grpc.TransactionVerificationServiceStub(channel)
        stub.ClearData(
            transaction_verification.ClearDataRequest(
...
async def clear_fraud_detection_data(order_id: str, final_vector_clock: dict):
    ...
    with grpc.insecure_channel("fraud_detection:50051") as channel:
        stub = fraud_detection_grpc.FraudDetectionServiceStub(channel)
        stub.ClearData(
            fraud_detection.ClearDataRequest(
