...
class SuggestionService(suggestions_grpc.SuggestionServiceServicer):
    def Initialize(self, request: suggestions.InitializationRequest, context):
        ...
        logging.info(
            f"Successfully initialized suggestions with vector clock: {vector_clock} \
                      and data: {request}"
        )
        return response

    def GenerateSuggestions(
        self, request: suggestions.GenerateSuggestionsRequest, context
    ):
        ...
        cached_data: tuple[dict, suggestions.InitializationRequest] = (
    ...
    def GetSuggestions(self, request: suggestions.GetSuggestionsRequest, context):
        ...
        cached_data: tuple[dict, suggestions.GetSuggestionsResponse] = (
            vector_clock_cache[request.order_id.value]
        )
